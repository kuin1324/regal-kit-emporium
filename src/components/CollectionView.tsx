    ...
    <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
      ...
      <aside className="lg:w-64 lg:shrink-0">
        <button
          ...
          className="flex w-full items-center justify-between gap-2 rounded-xl border border-border/50 bg-card/60 px-4 py-3 text-left transition-colors hover:border-primary/40"
        >
          ...
          {showFilters ? <X className="h-4 w-4 text-muted-foreground" /> : null}
        </button>
        {showFilters && (
          <div className="mt-3">
            <ProductFilters items={items} state={filters} onChange={(patch) => setFilters((f) => ({ ...f, ...patch }))} />
          </div>
        )}
      </aside>

      <div className="min-w-0 flex-1">
        {/* Zoekbalk: opent meteen de filters aan de linkerkant */}
          ...
          {pageItems.map((product, i) => (
            ...
            >
              ...
              <div
                ...
                className="relative overflow-hidden rounded border border-border/50 bg-card transition-all duration-500 group-hover:border-primary/30 group-hover:shadow-[var(--shadow-gold)]"
              >
