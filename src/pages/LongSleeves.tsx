      ...
      <section className="pt-28 pb-24">
        <div className="container mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center mb-12">
            <p className="text-xs font-medium tracking-[0.3em] uppercase text-primary mb-3">{t("longSleeves.eyebrow", { defaultValue: "Lange Mouwen" })}</p>
          ...
          <p className="text-center text-xs text-muted-foreground mt-4">
            {longSleeveProducts.length === 0 ? 0 : (page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, longSleeveProducts.length)} / {longSleeveProducts.length}
          </p>
        </div>
      </section>
      <Footer />
